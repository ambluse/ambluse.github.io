...
# 2. 拷贝文件
--
**拷贝文件 `boost_1_59.tar.gz`和 `mysql-5.6.26.tar.gz` 到/u01目录  
执行解压:**
[安装文件在这](https://github.com/ambluse/install-files/tree/master/MySQL%E7%9B%B8%E5%85%B3)
...
```
>bash_profile 内容也在最后
>注意运维脚本放到 mysql/local/bin中，这个在bash中也有配bin的目录，关注下。
>记得改脚本的密码
